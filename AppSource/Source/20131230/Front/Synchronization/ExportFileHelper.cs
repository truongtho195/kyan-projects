...
namespace CPC.POS.ViewModel.Synchronization
{
    public class ExportFileHelper
    {
        ...
        private static List<base_TempModel> _changedRecordList = new List<base_TempModel>();
        private static string _directoryName = new FileInfo(Assembly.GetExecutingAssembly().Location).DirectoryName+ @"\SyncFile\";
        private static string _databaseName = "pos";
        ...
        public static void ExportFile(string connection)
        {
            try
            {
                ...
                // Export file if has record changed
                if (_changedRecordList.Count > 0)
                {
                    CreateTempFile();
                    CreateScript();
                    CreateSynchronousFile(_subFileIndex > 1);

                    // Update all synchronized records in temp table
                    UpdateSynchronizedData();
            ...
            {
                Xceed.Wpf.Toolkit.MessageBox.Show(ex.ToString(), "Export File.");
            }
        }
        ...
        {
            string fileName = string.Format("{0}_{1}_temp.txt", _databaseName, DateTime.Now.AddTicks(1).ToString("yyMMddHHmmssfff"));
        ...
        private static void CreateScript()
        {
            ...
            {
                // Get column name list of each table
        ...
        private static void UpdateSynchronizedData()
        {
            ...
            string updateCommand = string.Format("UPDATE \"base_Temp\" SET \"SynchronizationDate\"=now(), \"IsSynchronous\"=true ");
        ...
        private static void ClearData()
        {
            ...
            _directoryName = new FileInfo(Assembly.GetExecutingAssembly().Location).DirectoryName + @"\SyncFile\";
