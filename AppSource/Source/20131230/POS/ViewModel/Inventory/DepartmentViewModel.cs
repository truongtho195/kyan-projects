    ...
    public class DepartmentViewModel : ViewModelBase
    {
        ...
        private bool SaveOrRestoreCategoriesWithCancel()
        {
            if (_selectedItem is DepartmentRootModel)
            {
                ...
                {
                    MessageBoxResult result = Xceed.Wpf.Toolkit.MessageBox.Show(Language.Text16, Language.Warning, MessageBoxButton.YesNoCancel, MessageBoxImage.Warning);
                    if (result == MessageBoxResult.Yes)
                    {
                        SaveDepartments(depRoot.ProductDepartmentCollection);
                    }
        ...
        private bool SaveOrRestoreCategories()
        {
            if (_selectedItem is DepartmentRootModel)
            {
                ...
                if (depRoot.ProductDepartmentCollection.IsDirty)
                {
                    MessageBoxResult result = Xceed.Wpf.Toolkit.MessageBox.Show(Language.Text16, Language.Warning, MessageBoxButton.YesNo, MessageBoxImage.Warning);
                    if (result == MessageBoxResult.Yes)
                    {
                        SaveDepartments(depRoot.ProductDepartmentCollection);
                    }
