    ...
    class PickPackViewModel : ViewModelBase
    {
            ...
            : this()
        {
            ...
            if (saleOrderDetailCollection.Any(x => x.DueQty > 0))
            {
                ...
                foreach (var item in saleOrderDetailCollection.CloneList())
                    SaleOrderDetailList.Add(item);

                SaleOrderDetailCollection = SaleOrderDetailList;

                foreach (var item in SaleOrderDetailCollection.Where(x => x.DueQty > 0))
                {
                    item.IsPickProcess = true;
                    item.QtyOfPick = item.DueQty;
                }
            ...
            : this()
        {
            ...
            foreach (var item in SaleOrderDetailCollection)
            {
                item.IsPickProcess = true;
                int qtyPacked = SaleOrderShipModel.SaleOrderShipDetailCollection.SingleOrDefault(x => x.SaleOrderDetailResource == item.Resource.ToString()).PackedQty.Value;
                item.PickQty -= qtyPacked;
                item.QtyOfPick = qtyPacked;
            }
        }
        ...
        private void OnOkCommandExecute()
        {
            if (!IsView)
            {
                ...
                {
                    foreach (base_SaleOrderDetailModel saleOrderDetailModel in SaleOrderDetailCollection.Where(x => x.QtyOfPick > 0))
                    {
