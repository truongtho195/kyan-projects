...
namespace CPC.Toolkit.Behavior
{
    public class TabItemGeneratorBehavior : INotifyPropertyChanged
    {
            ...
            set
            {
                if (_itemsSource != value)
                {
                    ...
                    if ((ItemsSource as IList).Count > 0)
                        SelectedTabItem = _tabControl.Items.Cast<TabItem>().FirstOrDefault();
                }
            }
        }

        private static TabItemGeneratorBehavior _instance;
        ...
        private void AddTabItem(object item)
        {
            ...
            if (!_tabControl.Items.Cast<TabItem>().Select(x => x.Header).Contains(tab.Header))
