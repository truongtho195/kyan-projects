...
{
    public class WorkOrderViewModel : OrderViewModel, IDropTarget
    {
        #region Define
        //Respository
        ...
        public WorkOrderViewModel(bool isList, object param)
            : base()
        {
            LoadDynamicData();
            ...
            AllowDeleteProduct = UserPermissions.AllowDeleteProductWorkOrder;
        }
        ...
        protected override void OnSearchProductCommandExecute(object param)
        {
            ...
            else if (_requireProductCard && ExtensionProducts.Any(x => x.Barcode.Equals(BarcodeProduct)))
            {
        ...
        private void OnDeleteItemsWithKeyCommandExecute(object param)
        {
            ...
            {
                Xceed.Wpf.Toolkit.MessageBox.Show(Language.GetMsg("SO_Message_CouldDeleteItemIsWorkOrder"), Language.GetMsg("InformationCaption"), MessageBoxButton.OK, MessageBoxImage.Warning, MessageBoxResult.OK);
                return;
            }
        ...
        private bool OnConvertToSaleOrderCommandCanExecute(object param)
        {
            ...
            return !SelectedSaleOrder.IsNew
                ...
                && UserPermissions.AllowSaleWorkOrder;
        }
        ...
        {
            return SelectedSaleOrder != null && SelectedSaleOrder.Deposit > 0 && (SelectedSaleOrder.Mark.Equals(MarkType.WorkOrder.ToDescription()) && !SelectedSaleOrder.IsConverted);
        }
        ...
        private void OnRefundCommandExecute(object param)
        {
            ...
            MessageBoxResult result = Xceed.Wpf.Toolkit.MessageBox.Show(msg, Language.GetMsg("InformationCaption"), MessageBoxButton.YesNo, MessageBoxImage.Information, MessageBoxResult.Yes);
        ...
        private void LoadDataByPredicate(Expression<Func<base_SaleOrder, bool>> predicate, bool refreshData = false, int currentIndex = 0)
        {
            ...
            bgWorker.DoWork += (sender, e) =>
            {
                ...
                predicateAll = predicateAll.And(x => !x.IsPurge && !x.IsLocked && x.Mark.Equals(workOrderMark)).And(predicate);
                ...
                lock (UnitOfWork.Locker)
                {
                    ...
                    //Get data with range
                    IList<base_SaleOrder> saleOrders = _saleOrderRepository.GetRange<long>(currentIndex - _numberNewItem, NumberOfDisplayItems, x => x.Id, predicateAll);
                    if (refreshData)
        ...
        private void OpenSOAdvanceSearch()
        {
            ...
            _salesOrderAdvanceSearchViewModel.LoadData("WorkOrder");
        ...
        public void Drop(DropInfo dropInfo)
        {
            if (dropInfo.Data is ComboItem)
            {
                (_ownerViewModel as MainViewModel).OpenViewExecute("Work Order", dropInfo.Data);
            }
        }
