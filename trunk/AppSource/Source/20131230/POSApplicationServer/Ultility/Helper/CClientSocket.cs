...
public class CClientSocket
{
    ...
    public CClientSocket(string IP, int port)
    {
        try
        {
            mPort = port;
            IPAddress ipAddress;
            bool res = TryParseAddress(IP, out ipAddress);
            if (res)
            {
    ...
    public bool SendByte(byte data)
    {
        try
        {
            ...
            buffer[0] = data;
            mainSocket.BeginSend(buffer, 0, buffer.Length, 0, new AsyncCallback(SendCallback), mainSocket);
            return true;
        }
        catch (ArgumentException se)
        {
    ...
    private bool TryParseAddress(string address, out IPAddress ip)
    {
