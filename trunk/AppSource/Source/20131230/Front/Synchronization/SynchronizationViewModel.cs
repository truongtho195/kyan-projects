...
namespace CPC.POS.ViewModel
{
    public class SynchronizationViewModel : ViewModelBase
    {
        ...
        private string LogPath = Define.CONFIGURATION.StoreFolder + @"\Logs\";
        private DateTime LastDate = DateTime.Now.AddDays(-10);
        protected string ConnectString = new EntityConnectionStringBuilder(ConfigurationManager.ConnectionStrings["POSDBEntities"].ConnectionString).ProviderConnectionString;
        ...
        private string DirectoryName = new FileInfo(Assembly.GetExecutingAssembly().Location).DirectoryName + @"\SyncFile\";
        #endregion
        ...
        public SynchronizationViewModel(int storeIndex)
        {
            ...
            this._serviceCallBack.ClientNotified += ChatServiceCallback_ClientNotified;
            this.ServiceClient = new ServiceClient(new InstanceContext(this._serviceCallBack));
            this.ServiceClient.Authentication(this.Store);
                ...
                switch (e.Message.ToString())
                {
                    ...
                    case "AuthenticationResult":
                        ...
                        if (this.IsConnection)
                            //Debug.WriteLine("Conneting store is successful");
                            Xceed.Wpf.Toolkit.MessageBox.Show("Conneting store is successful", this.Store.Name);
                        else
                            //Debug.WriteLine("Conneting store is failture.Please try it again.");
                            Xceed.Wpf.Toolkit.MessageBox.Show("Conneting store is failture.Please try it again.", this.Store.Name);
                        break;
                    ...
                    case "SendFile":
                        Xceed.Wpf.Toolkit.MessageBox.Show(string.Format("Server send a request that server will get data of {0}.", this.Store.Name), Language.Warning, MessageBoxButton.OK, MessageBoxImage.Warning);
                        try
                        {
                        ...
                        {
                            Xceed.Wpf.Toolkit.MessageBox.Show(ex.ToString(), "Server");
                        }
                        catch (Exception ex)
                        {
                    ...
                    case "Error":
                        ...
                        // Save file to folder
                        if (ErrorModel.ByteSize != null && ErrorModel.ByteSize.Count() > 0)
                        {
                            Xceed.Wpf.Toolkit.MessageBox.Show("Data Synchronization isn't completed.You should update status of this items.", ErrorModel.StoreName, MessageBoxButton.OK, MessageBoxImage.Warning);
                            this.GetFile(ErrorModel);
                            this.UpdateStatusFile(this.StorePath + ErrorModel.FileName);
                        }
                        else
                            Xceed.Wpf.Toolkit.MessageBox.Show("Data Synchronization is completed.");
                        break;

                    case "NoFile":
                        ExportFileHelper.SyncFilePathCollection = null;
                        ExportFileHelper.ExportFile(this.ConnectString);
                        if (ExportFileHelper.SyncFilePathCollection.Count == 0)
                            e.Result = true;
                        else
                            e.Result = false;
                        break;
                    ...
                    case "ServerDisconnect":
                        if (e.Result)
                            //Debug.WriteLine("Conneting store is successful");
                            Xceed.Wpf.Toolkit.MessageBox.Show("Main Store disconnected !", this.Store.Name);
                        break;
                }
            }
            catch (Exception ex)
        ...
        private bool CheckMainStore(StoreModel storeModel)
        {
            try
            {
                ...
                if (mainStore != null && storeModel.Password == mainStore.Password)
                {
        ...
        private List<QueryModel> GetQuery(string path)
        {
            ...
            try
            {
                ...
                foreach (System.IO.FileInfo file in fileNames.OrderBy(x => x.CreationTime).Where(x => x.CreationTime > LastDate && x.Extension.Equals(".zip")))
                {
                    ...
                    //Move file that doesn't compress to new path.
                    this.MoveFile(new FileInfo(file.FullName));
                    string newFile = OutPutFile.Replace(".zip", "");
                    List<string> line = System.IO.File.ReadAllLines(newFile).ToList();
                    content.AddRange(line);
                    //Move file that compressed to new path.
                    this.MoveFile(new FileInfo(newFile));
                }
        ...
        private void ImportData(FileInfoModel FileInfoModel)
        {
            ...
            try
            {
                ...
                using (NpgsqlConnection pgsqlConnection = new NpgsqlConnection(this.ConnectString))
                {
                    ...
                    if (Commands != null && Commands.Count > 0)
                    {
                        ...
                        {
                            string query = string.Format("ALTER TABLE \"{0}\" DISABLE TRIGGER \"process_product\"", itemTable.Text);
                        ...
                        foreach (var itemTable in this.Tables)
                        {
                            string query = string.Format("ALTER TABLE \"{0}\" ENABLE TRIGGER \"process_product\"", itemTable.Text);
                            using (NpgsqlCommand pgsqlcommand = new NpgsqlCommand(query, pgsqlConnection))
                            {
                                try
                                {
                ...
                if (failtureData.Count > 0)
                {
                    try
                    {
                        ...
                        ExportFileHelper.SyncFilePathCollection = new List<string>();
                        infoModel.ByteSize = File.ReadAllBytes(fileInfo.FullName + ".zip");
                        infoModel.Length = infoModel.ByteSize.LongLength;
                        infoModel.FileName = fileInfo.Name;
                        this.ServiceClient.SendResultToStore(infoModel);
                    }
                    catch (Exception ex)
                    {
                        Xceed.Wpf.Toolkit.MessageBox.Show(ex.ToString(), "Failture File.");
                    }
                }
                else
            ...
            catch (Exception msg)
            {
                ...
                Xceed.Wpf.Toolkit.MessageBox.Show(msg.ToString());
            }
        }

        private void UpdateSynchronizedData()
        {
        ...
        public void SendFile()
        {
            try
            {
                ...
                if (ExportFileHelper.SyncFilePathCollection.Count == 0)
                {
                    Xceed.Wpf.Toolkit.MessageBox.Show("No any store to synchrnize data.");
                    return;
                }
        ...
        public void RefreshStore()
        {
            try
            {
                ...
                if (this.ServiceClient.State == CommunicationState.Opened)
                {
                    var queryStores = this.StoreCollection.Where(x => x.Index != this.Store.Index).ToList();
                    for (int i = 0; i < queryStores.Count(); i++)
                    {
                        bool result = this.ServiceClient.RefreshStore(queryStores[i]);
                        if (!result)
                            this.StoreCollection.Remove(this.StoreCollection.SingleOrDefault(x => x.Index == queryStores[i].Index));
                    }
                }
            }
            catch (TimeoutException ex)
            {
                Xceed.Wpf.Toolkit.MessageBox.Show(ex.ToString(), "Server Refresh Store");
            }
            catch (Exception ex)
            {
                Xceed.Wpf.Toolkit.MessageBox.Show(ex.ToString(), "Server Refresh Store");
        ...
        public void CloseService()
        {
            try
            {
                ...
                this.ServiceClient.MainStoreDisConnect(this.Store);
                this.ServiceClient.Disconnect(this.Store);
                this.ServiceClient.Abort();
            }
            catch (Exception ex)
            {
        ...
        private void UpdateStatusFile(string path)
        {
            try
            {
                using (NpgsqlConnection pgsqlConnection = new NpgsqlConnection(this.ConnectString))
                {
                    // Open the PgSQL Connection.                
                    pgsqlConnection.Open();
                    List<QueryModel> Commands = this.GetQuery(path);
                    if (Commands != null && Commands.Count > 0)
                    {
                        foreach (var item in Commands)
                        {
                            string query = string.Format("UPDATE \"base_Temp\" SET \"IsSynchronous\"=true where \"Resource\"={0}", item);
                            using (NpgsqlCommand pgsqlcommand = new NpgsqlCommand(query, pgsqlConnection))
                            {
                                pgsqlcommand.CommandType = System.Data.CommandType.Text;
                                int result = pgsqlcommand.ExecuteNonQuery();
        ...
        private void MoveFile(FileInfo FileInfo)
        {
            try
            {
                string NewPath = FileInfo.Directory + @"\OldFile\";
            ...
            {
                Xceed.Wpf.Toolkit.MessageBox.Show(ex.ToString(), "Move File.");
            }
        }
        #endregion
